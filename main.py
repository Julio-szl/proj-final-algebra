...
class CalculadoraMatrices:
    def __init__(self, root, tamaño, operacion):
        self.root = root
        ...
        self.matrizA_entries = []
        self.matrizB_entries = []
    ...
    def crear_interfaz(self):
        titulo = ctk.CTkLabel(self.root, text="Calculadora de Matrices", font=("Arial", 24, "bold"))
        titulo.pack(pady=10)

        frame_opciones = ctk.CTkFrame(self.root)
        frame_opciones.pack(pady=10)

        ctk.CTkLabel(frame_opciones, text="Tamaño actual:").grid(row=0, column=0, padx=5)
        ctk.CTkLabel(frame_opciones, textvariable=self.size_matriz, font=("Arial", 16, "bold")).grid(row=0, column=1, padx=5)

        ctk.CTkLabel(frame_opciones, text="Operación:").grid(row=0, column=2, padx=5)
        ...
        self.frame_matrices = ctk.CTkFrame(self.root)
        self.frame_matrices.pack(pady=15)
        self.generar_campos(self.size_matriz.get())

        self.resultado_label = ctk.CTkTextbox(self.root, height=150, width=600, font=("Consolas", 18))
    ...
    def generar_campos(self, tamaño):
        ...
        for i in range(filas):
            ...
            for j in range(columnas):
                ...
                fila.append(entry)
            self.matrizA_entries.append(fila)
        ...
        if self.operacion_matriz.get() in ["Suma", "Resta", "Multiplicación"]:
            ...
            for i in range(filas):
                ...
                for j in range(columnas):
                    ...
                    entry.grid(row=i, column=j, padx=3, pady=3)
                    fila.append(entry)
                self.matrizB_entries.append(fila)

    def calcular(self):
        op = self.operacion_matriz.get()
        A = validar_numeros(self.matrizA_entries)
        if A is None:
            return

        if op in ["Suma", "Resta", "Multiplicación"]:
            B = validar_numeros(self.matrizB_entries)
            if B is None:
                return
        else:
            B = None
